((cssText, artDataUrl, videoDataUrl, petDataUrl, retroFrameDataUrl, qqAvatarDataUrl, coughAudioDataUrl, themeConfig) => {
  ...
  const BACKGROUND_VIDEO_ID = "codex-qq-skin-background-video";
  ...
  const THEME_VARIABLES = [
    ...
    "--dream-retro-frame", "--dream-summary-panel-width", "--dream-summary-panel-height",
  ];
  const installToken = {};
  ...
  const videoUrl = videoDataUrl ? dataUrlToObjectUrl(videoDataUrl, "video/mp4") : null;
  ...
  const weeklyUsageRefreshMs = 15000;
  let weeklyUsageLastRefreshAt = 0;
  ...
  const syncWeeklyUsage = (node) => {
    if (!node) return;
    const visibleAccount = findCurrentAccountIdentity();
    const refreshDue = Date.now() - weeklyUsageLastRefreshAt >= weeklyUsageRefreshMs;
    if (!refreshDue && node.dataset.visibleAccount === visibleAccount) return;
    const reactUsage = findReactWeeklyUsage();
    const accountIdentity = reactUsage?.accountIdentity || visibleAccount;
    ...
    const usageValue = remaining == null ? "--" : `${remaining}%`;
    ...
    weeklyUsageLastRefreshAt = Date.now();
  };
  ...
  const ensureCompanion = () => {
    ...
    if (!companion || companion.dataset.qqCompanionVersion !== "2") {
      companion?.remove();
      companion = document.createElement("section");
      companion.id = COMPANION_ID;
      companion.dataset.qqCompanionVersion = "2";
      ...
      companion.innerHTML = `
        ...
        <div class="qq-skin-weekly-usage"><span>本周剩余</span><strong>--</strong></div>`;
  ...
  const applyRootState = (root) => {
    ...
    root.toggleAttribute("data-qq-video-background", Boolean(videoUrl));
    setStyleProperty(root, "--qq-skin-art", videoUrl ? "none" : `url("${artUrl}")`);
  ...
  const syncRouteState = (shell, { layout = false } = {}) => {
    ...
    if (summaryPanel && typeof summaryPanel.getBoundingClientRect === "function") {
      ...
      const summaryHeight = Math.round(summaryBox.height);
      setStyleProperty(root, "--dream-summary-panel-width", `${summaryWidth}px`);
      setStyleProperty(root, "--dream-summary-panel-height", `${summaryHeight}px`);
      if (companion.parentElement !== summaryPanel.parentElement) {
        summaryPanel.insertAdjacentElement("afterend", companion);
      }
    } else {
      root.style.removeProperty("--dream-summary-panel-width");
      root.style.removeProperty("--dream-summary-panel-height");
    }
  ...
  const cleanup = () => {
    ...
    document.documentElement?.removeAttribute("data-qq-video-background");
  ...
  const routeStructureSelector = [
    "main.main-surface",
    "aside.app-shell-left-panel",
    ".composer-surface-chrome",
    '[data-pip-obstacle="thread-summary-panel"]',
  ].join(",");
  const hasRouteStructure = (node) => node?.nodeType === Node.ELEMENT_NODE && (
    node.matches?.(routeStructureSelector) || node.querySelector?.(routeStructureSelector)
  );
  const observer = new MutationObserver((mutations) => {
    for (const mutation of mutations) {
      for (const node of mutation.addedNodes) {
        if (hasRouteStructure(node)) return scheduleEnsure({ route: true });
      }
  ...
  if (previous?.videoUrl && previous.videoUrl !== videoUrl) URL.revokeObjectURL(previous.videoUrl);
  ...
  const startupResizePasses = new Set([1, 3, 6, 8]);
  let startupPass = 0;
